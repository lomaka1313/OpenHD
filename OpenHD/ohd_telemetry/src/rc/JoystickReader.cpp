...
int JoystickReader::process_event(void* event1,
  ...
  switch (event->type) {
    case SDL_JOYAXISMOTION:
      ...
      m_console->debug("Joystick {}, Axis {} moved to {}, mavlink value {}", event->jaxis.which,
                       event->jaxis.axis, event->jaxis.value, remap_sdl_to_mavlink(event->jaxis.value));
      ret = 2;
      break;
...
void JoystickReader::write_matching_axis(
  ...
  if (axis_index >= N_CHANNELS_RESERVED_FOR_AXES + N_CHANNELS_RESERVED_FOR_SLIDERS) {
  ...
  if (axis_index >= N_CHANNELS_RESERVED_FOR_AXES) {
    rc_data[axis_index + N_CHANNELS_RESERVED_FOR_BUTTONS] = remap_sdl_to_mavlink(value);
    return;
  }
  rc_data[axis_index] = remap_sdl_to_mavlink(value);
}

void JoystickReader::write_matching_button(std::array<uint16_t, N_CHANNELS>& rc_data,
                                           const uint8_t button, bool up) {
  bool pressed = !up;
  // якщо стабільний стан, але змінився — оновлюємо
  if (button_states[button] != pressed) {
    button_states[button] = pressed;
    for (const auto& [sw_id, sw] : switches) {
      if (std::find(sw.buttons.begin(), sw.buttons.end(), button) != sw.buttons.end()) {
        rc_data[sw.channel - 1] = calc_pwm_for_switch(sw);
      }
    }
  }
}

int JoystickReader::calc_pwm_for_switch(const SwitchConfig& sw) {
  if (sw.type == SwitchType::TWO_POS) {
    bool state = button_states[sw.buttons[0]];
    return state ? JoystickReader::VALUE_BUTTON_UP : JoystickReader::VALUE_BUTTON_DOWN;
  }

  if (sw.type == SwitchType::THREE_POS) {
    for (size_t i = 0; i < sw.buttons.size(); ++i) {
      if (button_states[sw.buttons[i]]) {
        switch (i) {
          case 0: return JoystickReader::VALUE_BUTTON_DOWN;
          case 1: return JoystickReader::VALUE_BUTTON_MIDDLE;
          case 2: return JoystickReader::VALUE_BUTTON_UP;
        }
      }
    }

    return JoystickReader::VALUE_BUTTON_MIDDLE;
  }
